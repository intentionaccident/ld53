...
export function processEvents(ship: Ship, setGloopAmount) {
	while (ship.eventQueue.length > 0) {
		...
		if (event.type === GameEventType.KeyPressed && event.key == ' ') {
			ship.gloopAmount += 10;
			setGloopAmount(ship.gloopAmount);
		} else if (event.type === GameEventType.KeyPressed && event.key == 'c') {
		...
		} else if (event.type === GameEventType.RotateIntersection) {
		...
		} else if (event.type === GameEventType.CounterRotateIntersection) {
			const room = ship.roomHandles[event.y][event.x];
			const previousTop = room.data.topOpen;
		...
		} else if (event.type === GameEventType.FeatureClicked) {
